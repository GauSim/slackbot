...
class WhatsOnline {
    ...
    formatDate(verb, stamp) {
        ...
        if (stamp.indexOf('GMT') > -1) {
            date = moment(new Date(stamp)).zone(zone);

        } else {
            date = moment.utc(new Date(stamp));
        }
        ...
        const dateStr = days >= 28 ? moment(date).fromNow() : moment(date).format('DD.MM HH:mm');
