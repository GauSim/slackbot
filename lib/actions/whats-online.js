...
const cloud = {
    ...
    'SANDBOX':'https://isb.dev.coresuite.com/dc/status'
}
...
class WhatsOnline {
    ...
    formatDate(verb, stamp) {
        ...
        let str = moment(stamp).format();
        let date = moment(str);
        if (stamp.indexOf('Z') > -1) {
            date = moment().utc(stamp);
        }
        ...
        */
       // const dateStr = days <= 28 ? moment(date).fromNow() : moment(date).format('DD.MM.YYYY HH:mm');
        return `${verb}: ${stamp}`;
    }
    ...
    generateErrorMsg(error, env, app, url) {
        if (this.doNotThrow) {
            return `<${url}|${env}> | could not find ${app} on ${env}`;
        } else {
