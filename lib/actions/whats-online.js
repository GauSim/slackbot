...
class WhatsOnline {
    ...
    getVersionFrontend(env, app, url) {
        ...
        return this.get({
            ...
            .then(({body, deployDate}) => {
                ...
                const versionStr = json.appConfig.version ? this.formatGitHubTag(app, json.appConfig.version) : NO_VERSION_NUMBER_FOUND;
    ...
    getVersionFacade(env, app, url) {
        ...
        return this.get(url)
            .then(rawBody => {
                ...
                hash = json.lastCommit;
                version = json.version ? json.version : NO_VERSION_NUMBER_FOUND;
    ...
    getUrls(envToCheck = []) {
        ...
        work = work.concat(
            Object.keys(dataCloud)
                .filter(envFilter)
                .map(env => this.getVersionCloud(env, ENV_NAME_DATA_CLOUD, dataCloud[env]))
        );

        // master cloud 
        work = work.concat(
            Object.keys(masterCloud)
                .filter(envFilter)
                .map(env => this.getVersionCloud(env, ENV_NAME_MASTER_CLOUD, masterCloud[env]))
        );
        ...
        work = work.concat(
            Object.keys(admin)
                .filter(envFilter)
                .map(env => this.getVersionCloud(env, ENV_NAME_ADMIN, admin[env]))
        );
        ...
        work = work.concat(
            Object.keys(facade)
                .filter(envFilter)
                .map(env => this.getVersionFacade(env, ENV_NAME_FACADE, facade[env]))
        );
            ...
            handler: (bot, message) => {
                ...
                Promise.all(work)
                    .then(results => {
                        ...
                        return grpByAppHash
                            .keys()
                            .value().length === [...Object.keys(apps), ENV_NAME_FACADE, ENV_NAME_DATA_CLOUD].length
                            ? '... looks the same to me =>' +envToCheck.map(it => '`' + it + '`').join(' == ')
                            : grpByAppHash
                                .map((list, key) => `${list[0].app} | ${list[0].version ? list[0].version : list[0].hash.substr(0, 20) + '...'} => ` + list.map(it => '`' + it.env + '`').join(' == '))
                                .sort()
                                .join('\n')
                                .value()
                    })
