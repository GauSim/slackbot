...
const environments = [
    'ET', 'QT', 'PT', 'PROD', 'PROD-QA-EU', 'PROD-QA-US', 'SANDBOX'
];
...
module.exports = {
    action: {
        ...
        help: `i will check each environment and tell you what version is deployed, you can also check for a specific env => ${environments.map(e => "`" + e + "`").join(', ')}`,
        handler: (bot, message) => {
            ...
            let envToCheck = envInputString ?
                (envInputString.indexOf(',') > 1 ? envInputString.split(',') : envInputString.split(' ')) // split commar or space
                    ...
                    .filter(e => !!e && environments.map(e => e.toLowerCase()).indexOf(e.toLowerCase()) > -1) // is known env?
                : environments;


            if (!envToCheck.length) {
                bot.reply(message, `mhm? unknown environment ... \n try without a specific env or some of ${environments.map(e => "`" + e + "`").join(', ')}`);
                return;
            }
