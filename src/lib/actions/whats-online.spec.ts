...
const allKnownEnvironments = WhatsOnlineModul.allKnownEnvironments;
...
describe('#whats-online', () => {
  ...
  it('should perform all remote calls and return a string msg', done => {
    ...
    const work = whatsOnline.getUrls(allKnownEnvironments);

    whatsOnline.check(work)
      .then(results => {
        assert.deepEqual(results.length, expectedCallCount, `result count is ${expectedCallCount}`);
